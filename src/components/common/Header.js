...
function Filters({ properties, filters, onFilterChange }) {
  // Get unique values from actual data, but limit regions and branches
  const getUniqueValues = (field) => {
    const values = [...new Set(properties.map(p => p[field]).filter(Boolean))].sort();
    
    // Limit regions to only Phoenix and Las Vegas
    if (field === 'region') {
      return values.filter(v => ['Phoenix', 'Las Vegas', 'Peoria', 'Scottsdale'].includes(v));
    }
    
    // Limit branches to only Phoenix and Las Vegas
    if (field === 'branch') {
      return values.filter(v => ['Phoenix', 'Las Vegas'].includes(v));
    }
